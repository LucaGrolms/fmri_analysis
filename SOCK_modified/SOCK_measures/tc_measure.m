function [IC, tc_matrix] = tc_measure(IC, dir_ICA)
...
%	2012-2013: Original version by Kaushik Bhaganagarapu
   ...
   % cd into the directory where the timecourse files live.
   if gui_flag == 0
   ...
   IC.tc_matrix = tc_matrix; % Save tc_matrix in the IC structure
   ...
   for i=1:no_ics
	...
	IC.tc_corr(i) = find(abs(tc_matrix_coeff(:,i)) == temp);
   end

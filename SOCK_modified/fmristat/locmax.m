...
for slice=1:numslices
   ...
   no_in_excurset = no_in_excurset + length(find(excurset)+(slice-1)*numpix);
   find(excurset)+(slice-1)*numpix; 
   % Local maxima:
   islm=(X>=X1)&(X>=X2)&(X>=X3)&(X>=X4)&(X>=X5)&(X>=X6) ...
      &((X>X1)+(X>X2)+(X>X3)+(X>X4)+(X>X5)+(X>X6)>0)&excurset;
   %lmvox=[lmvox; find(islm)+(slice-1)*numpix];
   lmvox=[lmvox; find(excurset)+(slice-1)*numpix];
